...
from ..attention_processor import FusedTripoSGAttnProcessor2_0, TripoSGAttnProcessor2_0, PartragAttnProcessor
...
class PartragDiTModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    ...
    def __init__(
        ...
        if len(global_attn_block_ids) > 0:
            ...
            for layer_id in range(num_layers):
                for attn_id in [1, 2]:
                    if layer_id in global_attn_block_ids:
                        ...
                        attn_processor_dict[f'blocks.{layer_id}.attn{attn_id}.processor'] = PartragAttnProcessor()
            ...
            # logger.info(f"Modified {modified_attn_processor} to PartragAttnProcessor")
    ...
    def forward(
        ...
        if self.enable_part_embedding:
            # Add part embedding
            #  :attention_kwargsnum_parts,batchpart
            num_parts = attention_kwargs.get("num_parts", None) if attention_kwargs is not None else None
            
            #  :partmax_num_parts(CUDA)
            max_part_idx = self.part_embedding.num_embeddings  # 8
            
            if num_parts is None:
                # num_parts,batch_sizeparts
                #  :batch_sizemax_part_idx,!
                batch_size = hidden_states.shape[0]
                #  embeddings: batch=48,  [0,1,2,3,4,5,6,7,0,1,2,...]
                part_indices = torch.arange(batch_size, device=hidden_states.device) % max_part_idx
                part_embedding = self.part_embedding(part_indices)  # (batch_size, D)
                ...
                for num_part in num_parts:
                    #  :num_part>8,embedding [0-7]
                    num_part_safe = min(int(num_part), max_part_idx)
                    part_indices = torch.arange(num_part_safe, device=hidden_states.device) % max_part_idx
                    part_embedding_single = self.part_embedding(part_indices)  # (n, D)
                    
                    # partsmax_part_idx,embeddings
                    if num_part > num_part_safe:
                        # num_part(num_part_safe)
                        repeat_times = (int(num_part) + num_part_safe - 1) // num_part_safe
                        part_embedding_single = part_embedding_single.repeat(repeat_times, 1)[:int(num_part)]
                    
                    part_embeddings.append(part_embedding_single)
                part_embedding = torch.cat(part_embeddings, dim=0) # (N, D)
                hidden_states = hidden_states + part_embedding.unsqueeze(dim=1) # (N, T+1, D)
            elif isinstance(num_parts, int):
                #  :num_parts>8,embedding [0-7]
                num_parts_safe = min(num_parts, max_part_idx)
                part_indices = torch.arange(num_parts_safe, device=hidden_states.device) % max_part_idx
                part_embedding = self.part_embedding(part_indices)  # (N, D)
                
                # partsmax_part_idx,embeddings
                if num_parts > num_parts_safe:
                    repeat_times = (num_parts + num_parts_safe - 1) // num_parts_safe
                    part_embedding = part_embedding.repeat(repeat_times, 1)[:num_parts]
                hidden_states = hidden_states + part_embedding.unsqueeze(dim=1) # (N, T+1, D)
            else:
                raise ValueError(
