...
class ObjaversePartDataset(torch.utils.data.Dataset):
    ...
    def _get_data_by_config(self, data_config):
        ...
        def _maybe_get_labels(obj):
            if obj is None:
                return None
            if isinstance(obj, dict):
                for key in ("part_labels", "labels"):
                    if key in obj:
                        return obj[key]
            return None

        part_labels = data_config.get('part_labels')
        if part_labels is None and 'part_labels_path' in data_config:
            try:
                with open(data_config['part_labels_path'], 'r') as f:
                    part_labels = json.load(f)
            except FileNotFoundError:
                part_labels = None
        if part_labels is None:
            part_labels = _maybe_get_labels(locals().get('surface_data'))
        if part_labels is None:
            part_labels = list(range(part_surfaces.shape[0])) if part_surfaces.shape[0] > 0 else [0]

        if isinstance(part_labels, np.ndarray):
            part_labels = part_labels.tolist()
        part_labels = torch.tensor(part_labels, dtype=torch.long)
        if part_labels.shape[0] != part_surfaces.shape[0]:
            raise ValueError(
                f"Mismatch between number of part labels ({part_labels.shape[0]}) and part surfaces ({part_surfaces.shape[0]}) "
