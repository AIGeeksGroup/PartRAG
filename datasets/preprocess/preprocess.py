...
if __name__ == '__main__':
    ...
    for mesh_name in tqdm(os.listdir(input_path)):
        ...
        os.system(f"python -m datasets.preprocess.mesh_to_point --input {mesh_path} --output {output_path}")
        # 2. Render images
        os.system(f"python -m datasets.preprocess.render --input {mesh_path} --output {output_path}")
        ...
        os.system(f"python -m datasets.preprocess.rmbg --input {export_rendering_path} --output {output_path}")
        # 4. (Optional) Calculate IoU
        os.system(f"python -m datasets.preprocess.calculate_iou --input {mesh_path} --output {output_path}")
        time.sleep(1)
