...
def main():
    ...
    contrastive_part_weight = configs["train"].get("contrastive_part_weight", 0.0)
    contrastive_object_weight = configs["train"].get("contrastive_object_weight", 0.0)
    ...
    for batch in yield_forever(train_loader):
        ...
        with accelerator.accumulate(transformer):
            ...
            part_labels = batch["part_labels"].to(device=accelerator.device)
            ...
            with torch.no_grad():
                ...
                mesh_part_features = mesh_tokens.mean(dim=1).to(dtype=torch.float32)
                mesh_global = torch.stack([
                    chunk.mean(dim=0) for chunk in torch.split(mesh_part_features, num_parts.tolist())
                ], dim=0)

            # Extract per-object and per-part image embeddings
            image_part_features = image_embeds.mean(dim=1).to(dtype=torch.float32)
            image_global = torch.stack([
                chunk.mean(dim=0) for chunk in torch.split(image_part_features, num_parts.tolist())
            ], dim=0)
            ...
            if contrastive_part_weight > 0:
                part_contrastive_value = contrastive_loss.compute_symmetric(
                    image_part_features,
                    mesh_part_features,
                    labels_query=part_labels,
                    labels_key=part_labels,
                )
            ...
            if contrastive_object_weight > 0:
                object_contrastive_value = contrastive_loss.compute_symmetric(
                    image_global,
                    mesh_global,
                    labels_query=object_labels_global,
                    labels_key=object_labels_global,
                )
        ...
        if accelerator.sync_gradients:
            ...
            if (
                ...
                # Clean up old checkpoints BEFORE saving to avoid disk space issues
                if accelerator.is_main_process:
                    checkpoints = sorted([d for d in os.listdir(ckpt_dir) if d.isdigit()])
                    if len(checkpoints) > 0:  # Delete all old checkpoints before saving new one
                        for old_ckpt in checkpoints:
                            old_ckpt_path = os.path.join(ckpt_dir, old_ckpt)
                            import shutil
                            shutil.rmtree(old_ckpt_path, ignore_errors=True)
                            logger.info(f"Deleted old checkpoint before saving: {old_ckpt}")
                accelerator.wait_for_everyone()
                gc.collect()
                
                logger.info(f"Saving current state to {os.path.join(ckpt_dir, f'{global_update_step:06d}')}")
