...
if __name__ == "__main__":
    ...
    # and download upstream weights only when needed.
    partrag_weights_dir = resolve_or_download_weights(
        preferred_dir="/root/autodl-tmp/PartRAG/pretrained_weights/PartRAG",
    ...
    rmbg_weights_dir = resolve_or_download_weights(
        preferred_dir="/root/autodl-tmp/PartRAG/pretrained_weights/RMBG-1.4",
        repo_id="briaai/RMBG-1.4",
        legacy_dirs=("/root/autodl-tmp/PartCrafter/pretrained_weights/RMBG-1.4",),
        required_files=("config.json",),
        local_files_only=args.local_files_only,
    )
    print(f"Using pretrained weights: {partrag_weights_dir}")
    ...
    pipe: PartragPipeline = PartragPipeline.from_pretrained(partrag_weights_dir).to(device, dtype)
