...
if __name__ == "__main__":
    ...
    # Use local PartRAG path first, keep local legacy PartCrafter paths for compatibility,
    # and download upstream scene weights only when needed.
    partrag_scene_weights_dir = resolve_or_download_weights(
        preferred_dir="pretrained_weights/PartRAG-Scene",
        repo_id="wgsxm/PartCrafter-Scene",
        legacy_dirs=(
            "pretrained_weights/PartCrafter-Scene",
            "/root/autodl-tmp/PartCrafter/pretrained_weights/PartCrafter-Scene",
        ),
    ...
    pipe: PartragPipeline = PartragPipeline.from_pretrained(partrag_scene_weights_dir).to(device, dtype)
